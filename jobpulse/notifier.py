...
def send_discord_dm(new_jobs, csv_filename):
    ...
    try:
        ...
        # Build a readable markdown message
        lines = [f"🚀 **JobPulse Report**", f"Identified **{new_jobs_count}** new matching jobs today!\n"]
        
        # Discord has a 2000 char limit. We'll show up to 15 jobs in the text.
        display_limit = 15
        for job in new_jobs[:display_limit]:
            lines.append(f"• **{job['company']}**: [{job['title']}]({job['url']})")
            
        if new_jobs_count > display_limit:
            lines.append(f"\n*...and {new_jobs_count - display_limit} more in the attached CSV.*")
            
        message_content = "\n".join(lines)
        
        # Check if we accidentally exceeded 2000 chars (URLs can be long)
        if len(message_content) > 1950:
            message_content = message_content[:1950] + "\n\n*(Message truncated due to length, see CSV for full list)*"

        # 2. Send the message with the CSV attached
